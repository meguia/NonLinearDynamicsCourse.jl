...
# ╔═╡ f64c0520-ee22-11ee-0141-c77b069af2fc
using OrdinaryDiffEq, StaticArrays, Plots

# ╔═╡ 99cd8ba4-b573-47da-b1b9-1c086eb8d299
clist = [:black,:red,:blue,:yellow,:green,:purple,:cyan,:orange]

# ╔═╡ 9646d125-5eda-4d5a-9208-9cf78e381574
...
# ╔═╡ efa23520-d015-4e2c-bbaa-48fc41fb8893
...
# ╔═╡ 5a671198-e071-4c0e-9c2e-7dd576446f1a
...
# ╔═╡ c89a9f65-3807-4c0a-999b-fa39cca4bc48
begin
	u0 = [1.1; 0.1; 0.0]
	p = [0.11, 1.0, 0.1, 1.0]
	tspan = (0.0, 1000.0)
...
# ╔═╡ 07142147-ced7-4cc5-ba19-1b5d65a42392
begin
	(dx,dy) = [0.03, 0.03]
	(xmax,ymax) = [3.0, 2.0]
	x0 = -xmax:dx:xmax
	y0 = -ymax:dy:ymax
	(Nx,Ny) = length.([x0,y0])
	prob_func = (prob, i, repeat; x0=x0, y0=y0) -> remake(prob, u0 = [x0[mod(i-1,Nx)+1]; y0[(i-1)÷Nx+1]; 0.0])
	monteprob = EnsembleProblem(prob, prob_func = prob_func, safetycopy = false)
end	

# ╔═╡ e43e8a1b-8448-4505-b9de-0855a5d5e964
sol = solve(monteprob, Tsit5(), EnsembleThreads(), trajectories = Nx*Ny)

# ╔═╡ e308a388-8807-4fac-99de-d46ecf167b88
histp = find_attractors(sol,0.01,xmax,ymax);

# ╔═╡ b96f7eab-dabd-4e2f-a657-1905b2c15aa2
heatmap(log10.((histp .+ 1)/(Nx*Ny)),colorbar=false)

# ╔═╡ 37ed4bab-ed3c-4b4b-bf43-5fd07f784e4e
attractors=[[1.0,-0.1],[-1.0,-0.1],[0.9,0.65],[-0.4,0.27]];

# ╔═╡ 2892b3fc-4524-4fb8-9de5-350c09c8895a
Mbasin1 = basins(sol,Nx,Ny,attractors,0.3);

# ╔═╡ 3ffdebba-5cdd-4191-8407-49ba6a9f8b09
contourf(x0,y0,Mbasin1,size=(1200,800),c=clist[1:5],levels=-0.5:4.5,lw=0,colorbar=false)

# ╔═╡ 9884e361-b564-4018-b345-2982d56425e3
...
manifest_format = "2.0"
project_hash = "7195591f27afbd37d861f95b83e879a0de885efa"
